...
from onpolicy.runner.shared.base_runner_SAEIR import Runner
...
class FootballRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                # Sample actions
                values_team, values_group, values_idv, actions, action_log_probs, rnn_states, \
                rnn_states_critic_team, rnn_states_critic_group, rnn_states_critic_idv, actions_env, act_logits = self.collect(step)
                    
                # Obser reward and next obs
                obs, rewards, dones, infos = self.envs.step(actions_env)

                data = obs, rewards, dones, infos, values_team, values_group, values_idv, actions, \
                action_log_probs, rnn_states, rnn_states_critic_team, rnn_states_critic_group, rnn_states_critic_idv, act_logits 
    ...
    def collect(self, step):
        ...
        values_team, values_group, values_idv, actions, action_log_probs, rnn_states, rnn_states_critic_team, \
        rnn_states_critic_group, rnn_states_critic_idv, act_logits = self.trainer.policy.get_actions(
            np.concatenate(self.buffer.share_obs[step]),
            np.concatenate(self.buffer.obs[step]),
            np.concatenate(self.buffer.rnn_states[step]),
            np.concatenate(self.buffer.rnn_states_critic_team[step]),
            np.concatenate(self.buffer.rnn_states_critic_group[step]),
            np.concatenate(self.buffer.rnn_states_critic_idv[step]),
            np.concatenate(self.buffer.masks[step])
        )

        # [n_envs*n_agents, ...] -> [n_envs, n_agents, ...]
        values_team = np.array(np.split(_t2n(values_team), self.n_rollout_threads))
        values_group = np.array(np.split(_t2n(values_group), self.n_rollout_threads))
        values_idv = np.array(np.split(_t2n(values_idv), self.n_rollout_threads))
        actions = np.array(np.split(_t2n(actions), self.n_rollout_threads))
        action_log_probs = np.array(np.split(_t2n(action_log_probs), self.n_rollout_threads))
        rnn_states = np.array(np.split(_t2n(rnn_states), self.n_rollout_threads))
        rnn_states_critic_team = np.array(np.split(_t2n(rnn_states_critic_team), self.n_rollout_threads))
        rnn_states_critic_group = np.array(np.split(_t2n(rnn_states_critic_group), self.n_rollout_threads))
        rnn_states_critic_idv = np.array(np.split(_t2n(rnn_states_critic_idv), self.n_rollout_threads))

        actions_env = [actions[idx, :, 0] for idx in range(self.n_rollout_threads)]

        return values_team, values_group, values_idv, actions, action_log_probs, rnn_states, rnn_states_critic_team, \
        rnn_states_critic_group, rnn_states_critic_idv, actions_env, act_logits

    def insert(self, data):
        obs, rewards, dones, infos, values_team, values_group, values_idv, actions, action_log_probs, rnn_states, \
        rnn_states_critic_team, rnn_states_critic_group, rnn_states_critic_idv, act_logits = data
        ...
        rnn_states_critic_team[dones_env == True] = np.zeros(((dones_env == True).sum(), self.num_agents, self.recurrent_N, self.hidden_size), dtype=np.float32)
        rnn_states_critic_group[dones_env == True] = np.zeros(((dones_env == True).sum(), self.num_agents, self.recurrent_N, self.hidden_size), dtype=np.float32)
        rnn_states_critic_idv[dones_env == True] = np.zeros(((dones_env == True).sum(), self.num_agents, self.recurrent_N, self.hidden_size), dtype=np.float32)
        ...
        self.buffer.insert(
            ...
            rnn_states_critic_team=rnn_states_critic_team,
            rnn_states_critic_group=rnn_states_critic_group,
            rnn_states_critic_idv=rnn_states_critic_idv,
            actions=actions,
            action_log_probs=action_log_probs,
            value_team_preds=values_team,
            value_group_preds=values_group,
            value_idv_preds=values_idv,
            rewards=rewards,
            masks=masks
        )
